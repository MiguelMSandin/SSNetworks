...
parser = argparse.ArgumentParser(description="Creates a network file from a similarity blast output. Files will be exported to input_file_name followed by the identity threshold, the cover if selected, and the '.net' extension (i.e.; 'file_80.net', 'file_85i_80c.net').")
...
for i in iden.split('+'):
    if args.cover is not None:
        ...
        tmp = re.sub("\..*$", "", args.file_out) + "_" + i + "i" + "_" + args.cover + "c." + re.sub(".*\.", "", args.file_out)
    else:
