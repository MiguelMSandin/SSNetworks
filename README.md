...
For this workshop I would recommend using a relatively small subset of your data (~<4-mB fasta file formatted), in order to speed up computational analyses.  
...
>**Note**: You can also use the provided file, containing a random(ish) selection of 18S Radiolaria sequences trying to cover most of their diversity plus some Phaeodaria sequences as an outgroup (so you can also use the same file in phylogenetic analyses for comparisons). The reason why I haven't selected specific sequences is simply to expect unexpected outputs.  
...
### statsNetworks.R  
Finally we can make sense of all the previous analysis and get to see the results with the R script 'statsNetworks.R'. The script is self explained, so please read carefully the headers and comments so you know what you are doing.  
